...
class FeedbackGenerator(nn.Module):
	...
	def fuse_input_poses_multiple_frames(self, embed_poses_A, embed_poses_B):
		z = self.comparison_module(embed_poses_A, embed_poses_B)
		# z = self.modalityInputAdapter(z)
		return z
	...
	def multiple_frame_text_generation(self, poses_A, poses_B):
		pose_difference_stack = torch.zeros(16,32)
		for i in range(16):
			z_a = self.encode_pose(poses_A[i].view(1, -1, 3))
			z_b = self.encode_pose(poses_B[i].view(1, -1, 3))
			z = self.fuse_input_poses_multiple_frames(z_a, z_b)
			pose_difference_stack[i] = z
		
		pose_difference_stack = pose_difference_stack.view(1, 1, -1)
		
		decoded_texts, likelihood_scores = self.text_decoder.generate_greedy(pose_difference_stack)
		return decoded_texts, likelihood_scores
...
class ComparisonModule(nn.Module):
	...
	def __init__(self, inlatentD, outlatentD, mode="tirg"):
		...
		if mode == "tirg":
			self.tirg = TIRG(input_dim=[inlatentD, inlatentD], output_dim=outlatentD, out_l2_normalize=False)#[32,32],32,false
